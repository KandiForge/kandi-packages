...
export default function KandiLoginTestPage() {
  ...
  async function runAllTests() {
    ...
    await runTest(2, 0, async () => {
      ...
      // or 302/307 if CORS allows it
      if (res.type === 'opaqueredirect' || res.status === 302 || res.status === 307) return;
      // Some servers return 200 with a Location body for CORS compatibility
      if (res.status === 200) {
    ...
    await runTest(5, 0, async () => {
      ...
      // On re-seed, personas should be skipped (already exist) or re-created
      // The key test: login-as still works after re-seed
    ...
    await runTest(5, 1, async () => {
      // Validate resolves user from sub claim via getUserById
      const res = await fetch(`${base}/validate`, {
        headers: { Authorization: `Bearer ${accessToken}` },
        signal: abortRef.current?.signal,
      });
      ...
      // Verify the id matches the sub claim in the token
    ...
    await runTest(5, 2, async () => {
      ...
      if (!user.name && !user.display_name) warnings.push('Consider returning name or display_name');
